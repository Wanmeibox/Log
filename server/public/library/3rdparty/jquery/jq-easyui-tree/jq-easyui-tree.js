YUI.add("jq-easyui-tree", function(Y){
...
function _bindTreeEvents(_d){
	var _e=$.data(_d,"tree").options;
	var _tree=$.data(_d,"tree").tree;
	$("div.tree-node",_tree).unbind(".tree").bind("dblclick.tree",function(){
		_selectNode(_d,this);
		_e.onDblClick.call(_d,_8b(_d));
	}
	).bind("click.tree",function(e){
		_selectNode(_d,this,e.ctrlKey);
		...
		_e.onContextMenu.call(_d,e,_getNode(_d,this));
	}
	);
	$("span.tree-hit",_tree).unbind(".tree").bind("click.tree",function(){
		var _10=$(this).parent();
		_68(_d,_10[0]);
		return false;
	}
	...
	$("span.tree-checkbox",_tree).unbind(".tree").bind("click.tree",function(){
		var _parentNode=$(this).parent();
		_checkNode(_d,_parentNode[0],!$(this).hasClass("tree-checkbox1"));
		return false;
	}
	).bind("mousedown.tree",function(){
		return false;
		...
		////-----------zhyc unbind tree click
			$("span.tree-radio", _tree).unbind(".tree").bind("click.tree", function(){
				var _parentNode = $(this).parent();
				_checkRadioNode(_d, _parentNode[0], true);
				return false;
			}).bind("mousedown.tree", function(){
				return false;	
			});
...
function _12(_13){
	...
	_15.find("div.tree-node").draggable({
		revert:true,cursor:"pointer",proxy:function(_16){
...
function _1f(_21,_22){
	if(_getNode(_13,_22).state=="closed"){
...
function _checkNode(_2b,_2c,_2d){
	var opts=$.data(_2b,"tree").options;
	/////------------zhyc 
	if(opts.radio) {
		return _checkRadioNode(_2b, _2c, _2d);
	}
	if(!opts.checkbox){
		return;
	}
	...
	var _32=_getNode(_2b,_2c);
	opts.onCheck.call(_2b,_32,_2d);
	function _31(_34){
...
//////-------zhyc checkRadioNode
function _checkRadioNode(target, param, checked) {
			
			var opts = $.data(target, "tree").options;
			if (!opts.radio) {
				return;
			}
			$(target).find(".tree-radio").each(function() {
				var node = $(this);
				node.removeClass("tree-radio1");
				node.addClass("tree-radio0");
			});
			var node = $(param);
			var ck = node.find(".tree-radio");
			ck.removeClass("tree-radio0 tree-radio1");
			if (checked) {
				ck.addClass("tree-radio1");
			} else {
				ck.addClass("tree-radio0");
			}

			var nodeObj = _getNode(target, param);
			opts.onRadioCheck.call(target, nodeObj, checked);
		};
function _updateNodeCheckNode(_3b,_3c){
	var _3d=$.data(_3b,"tree").options;
	var _3e=$(_3c);
	if(_isLeafNode(_3b,_3c)){
		var ck=_3e.find(".tree-checkbox");
		if(ck.length){
			if(ck.hasClass("tree-checkbox1")){
				_checkNode(_3b,_3c,true);
			}
			else{
				_checkNode(_3b,_3c,false);
			}
		}
		else{
...
function _loadData(_target,ul,_46,_47){
	...
	_createChildNode(ul,_46,_4a);
	_bindTreeEvents(_target);
	_12(_target);
	for(var i=0;i<_checkedNodes.length;i++){
		_checkNode(_target,_checkedNodes[i],true);
	}
	var _4c=null;
	if(_target!=ul){
		var _4d=$(ul).prev();
		_4c=_getNode(_target,_4d[0]);
	}
	_opts.onLoadSuccess.call(_target,_4c,_46);
	function _createChildNode(ul,_data,_level){
		for(var i=0;i<_data.length;i++){
			var _nodeRoot=$("<li></li>").appendTo(ul);
			var _oneNode=_data[i];
			if(_oneNode.state!="open"&&_oneNode.state!="closed"){
				_oneNode.state="open";
			}
			var _nodeObj=$("<div class=\"tree-node\"></div>").appendTo(_nodeRoot);
			_nodeObj.attr("node-id",_oneNode.id);
			$.data(_nodeObj[0],"tree-node",{
				id:_oneNode.id,text:_oneNode.text,iconCls:_oneNode.iconCls,attributes:_oneNode.attributes
			});
			$("<span class=\"tree-title\"></span>").html(_oneNode.text).appendTo(_nodeObj);
			
			
			
			if(_oneNode.children&&_oneNode.children.length){
				// zhyc checked children
				var _childrenRoot=$("<ul></ul>").appendTo(_nodeRoot);
				if(_oneNode.state=="open"){
					$("<span class=\"tree-icon tree-folder tree-folder-open\"></span>").addClass(_oneNode.iconCls).prependTo(_nodeObj);
					addCheckBoxIcon(_oneNode, _nodeObj);
					$("<span class=\"tree-hit tree-expanded\"></span>").prependTo(_nodeObj);
				}
				else{
					$("<span class=\"tree-icon tree-folder\"></span>").addClass(_oneNode.iconCls).prependTo(_nodeObj);
					addCheckBoxIcon(_oneNode, _nodeObj);
					$("<span class=\"tree-hit tree-collapsed\"></span>").prependTo(_nodeObj);
					_childrenRoot.css("display","none");
				}
				_createChildNode(_childrenRoot,_oneNode.children,_level+1);
			}
			else{
				if(_oneNode.state=="closed"){
					$("<span class=\"tree-icon tree-folder\"></span>").addClass(_oneNode.iconCls).prependTo(_nodeObj);
					addCheckBoxIcon(_oneNode, _nodeObj);
					$("<span class=\"tree-hit tree-collapsed\"></span>").prependTo(_nodeObj);
				}
				else{
					$("<span class=\"tree-icon tree-file\"></span>").addClass(_oneNode.iconCls).prependTo(_nodeObj);
					addCheckBoxIcon(_oneNode, _nodeObj);
					$("<span class=\"tree-indent\"></span>").prependTo(_nodeObj);
				}
			}
	...
	function addCheckBoxIcon(_oneNode, _nodeObj) {
		if(_opts.checkbox){
				if(_opts.onlyLeafCheck){
					if(_oneNode.state=="open"&&(!_oneNode.children||!_oneNode.children.length)){
						if(_oneNode.checked){
							$("<span class=\"tree-checkbox tree-checkbox1\"></span>").prependTo(_nodeObj);
						}
						else{
							$("<span class=\"tree-checkbox tree-checkbox0\"></span>").prependTo(_nodeObj);
						}
					}
				}
				else{
			...
			///------zhyc add checked radio when load
			if(_opts.radio){
				if(_opts.onlyLeafRadio){
					if(_oneNode.state=="open"&&(!_oneNode.children||!_oneNode.children.length)){
						if(_oneNode.checked){
							$("<span class=\"tree-radio tree-radio1\"></span>").prependTo(_nodeObj);
						}
						else{
							$("<span class=\"tree-radio tree-radio0\"></span>").prependTo(_nodeObj);
						}
					}
				}
				else{
...
function _53(_54,ul,_55,_56){
	...
	$.ajax({
		type:_57.method,url:_57.url,data:_55,dataType:"json",success:function(_5b){
			_5a.removeClass("tree-loading");
			if(_57.onAfterLoad){
				_57.onAfterLoad(_5b);
			}
			
			_loadData(_54,ul,_5b);
			if(_56){
				_56();
			}
		}
...
function _73(_85,_86){
	...
	if(ul[0]==_85){
		...
		return _getNode(_85,ul.prev()[0]);
	}
};
////-------------zhyc get selected node
	function _getCheckedNode(target) {
			var opts = $.data(target, "tree").options, checked = false;
			if (opts.radio) {
				var checkedNodes = [];
				var checkedNode = null;
				$(target).find(".tree-radio1").each(function() {
					var parentNode = $(this).parent();
					checkedNodes.push(_getNode(target, parentNode[0]));
					//checkedNode = _getNode(target, parentNode[0]);
				});
				return checkedNodes;
			}
			if(opts.checkbox){
...
/*
function _getCheckedNode(_88){
	var _89=[];
	$(_88).find(".tree-checkbox1").each(function(){
		var _8a=$(this).parent();
		_89.push(_getNode(_88,_8a[0]));
	});
	return _89;
};
*/
...
function _8e(_8f,_90){
	...
	_loadData(_8f,ul[0],_90.data,true);
	_updateNodeCheckNode(_8f,ul.prev());
};
...
function _93(_94,_95){
	...
	if(_96){
		_8e(_94,{
			parent:_96.target,data:[_95.data]
		}
		);
...
function _97(_98,_99){
	...
	if(_9a){
		_updateNodeCheckNode(_98,_9a.target);
	}
};
...
function _updateNode(_a4,_a5){
	...
	ck.removeClass("tree-checkbox0 tree-checkbox1 tree-checkbox2");
	if(_a7.checked){
		_checkNode(_a4,_a5.target,true);
	}
	else{
		_checkNode(_a4,_a5.target,false);
	}
};

function _getNode(target, param) {
			var checked = $(param).find(".tree-checkbox").hasClass("tree-checkbox1");
			if($.data(target, "tree")){
				var opts = $.data(target, "tree").options; 
				//////-----------zhyc checked Radio
				if (opts.radio) {
					checked = $(param).find(".tree-radio").hasClass("tree-radio1");
				}
				if(opts.checkbox || opts.cascadeCheck || opts.onlyLeafCheck){
					checked = $(param).find(".tree-checkbox").hasClass("tree-checkbox1")
				}
			}
			var _ad = $.extend({}, $.data(param, "tree-node"), {
				target:param,
				checked: checked
			});
			if (!_isLeafNode(target, param)) {
				_ad.state = $(param).find(".tree-hit").hasClass("tree-expanded") ? "open" : "closed";
			}
			return _ad;
		}
/*
function _getNode(_a8,_a9){
	var _aa=$.extend({
	}
	,$.data(_a9,"tree-node"),{
		target:_a9,checked:$(_a9).find(".tree-checkbox").hasClass("tree-checkbox1")
	}
	);
	if(!_isLeafNode(_a8,_a9)){
		_aa.state=$(_a9).find(".tree-hit").hasClass("tree-expanded")?"open":"closed";
	}
	return _aa;
};
*/		
function _findNode(_ac,id){
	var _ad=$(_ac).find("div.tree-node[node-id="+id+"]");
	if(_ad.length){
		return _getNode(_ac,_ad[0]);
	}
	else{
		return null;
	}
		 ...
		 */
function _selectNode(_af,_b0,_c0){
	var _b1=$.data(_af,"tree").options;
	var _b2=_getNode(_af,_b0);
...
function _isLeafNode(_b3,_b4){
	var _b5=$(_b4);
	var hit=_b5.children("span.tree-hit");
	return hit.length==0;
};
$.fn.tree=function(_options,_params){
	if(typeof _options=="string"){
		return $.fn.tree.methods[_options](this,_params);
	}
	...
	return this.each(function(){
		...
		var _b9;
		if(_b8){
			_b9=$.extend(_b8.options,_options);
			_b8.options=_b9;
		}
		else{
			_b9=$.extend({
			...
			$.data(this,"tree",{
				options:_b9,tree:_1(this)
			}
			);
			var _ba=_4(this);
			_loadData(this,this,_ba);
		}
		if(_b9.data){
			_loadData(this,this,_b9.data);
		}
		if(_b9.url){
			_53(this,this);
		}
...
return jq.each(function(){
	_loadData(this,this,_bb);
}
);
}
,getNode:function(jq,_bc){
return _getNode(jq[0],_bc);
}
...
return _getCheckedNode(jq[0]);
}
,getSelected:function(jq){
return _8b(jq[0]);
}
,isLeaf:function(jq,_c2){
return _isLeafNode(jq[0],_c2);
}
,find:function(jq,id){
return _findNode(jq[0],id);
}
,select:function(jq,_c3){
return jq.each(function(){
	_selectNode(this,_c3);
}
);
}
,check:function(jq,_c4){
return jq.each(function(){
	_checkNode(this,_c4,true);
}
);
}
,uncheck:function(jq,_c5){
return jq.each(function(){
	_checkNode(this,_c5,false);
}
);
}
...
$.fn.tree.defaults={
	...
	onlyLeafRadio: false,
	dnd:false, data:null,
	onBeforeLoad:function(_d3,_d4){},
	onAfterLoad:function(_data){},
	...
	onRadioCheck:function(node, checked){},
...
, "0.0.1",{
requires: ["jquery","jquery-migrate", "jq-easyui-droppable", "jq-easyui-draggable", "jq-easyui-tree-css"]
}
);
