YUI.add("yam-control-tooltip", function(Y){
	var Y2 = Y.YAM,
		Lang = Y.Lang,
		Node = Y.Node,
		OX = -10000,
	...
	var Tooltip = Y.Base.create("tooltip", Y.Widget, [Y.WidgetPosition, Y.WidgetStack], {
		...
		_onNodeMouseMove : function(e) {
			var titlewidth = Y.one('#'+this.get('id')).get('offsetWidth');
			var width = this.get("delegate").get('offsetWidth');
			if(width-titlewidth-20<e.pageX){
				this._overTrigger(e.pageX-titlewidth-20, e.pageY);
			}else{
...
}, "0.0.1", {requires: ["yam-core","event-mouseenter", "widget", "widget-position", "widget-stack"]});